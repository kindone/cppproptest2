...
PROPTEST_API Shrinkable<T> shrinkIntegral(T value)
{
    if constexpr(is_same_v<T, int64_t>)
        ...
        return util::binarySearchShrinkable(static_cast<int64_t>(value)).map<T, int64_t>([](const int64_t& val) { return static_cast<T>(val); });
    else
        return util::binarySearchShrinkableU(static_cast<uint64_t>(value)).map<T, uint64_t>([](const uint64_t& val) { return static_cast<T>(val); });
}
