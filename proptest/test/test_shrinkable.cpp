...
TEST(Shrinkable, filter_exhaustive)
{
    ...
    Shrinkable<int64_t> shr2 = util::binarySearchShrinkable(8).filter([](const int64_t& val) { return val > 5; });
...
TEST(ShrinkableAny, map)
{
    ShrinkableAny shr(util::binarySearchShrinkable(8));
    EXPECT_EQ(shr.getAny().getRef<int64_t>(), 8);
    Shrinkable<int> shr2 = shr.map<int>([](const Any& val) -> int { return static_cast<int>(val.getRef<int64_t>() + 1); });
...
TEST(ShrinkableAny, flatMap)
{
    ShrinkableAny shr(util::binarySearchShrinkable(8));
    EXPECT_EQ(shr.getAny().getRef<int64_t>(), 8);
    Shrinkable<string> shr2 = shr.flatMap<string>([](const Any& val) -> Shrinkable<string> { return make_shrinkable<string>(to_string(val.getRef<int64_t>() + 1)); });
