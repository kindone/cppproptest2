...
TEST(Shrinkable, basic)
{
    ...
    EXPECT_EQ(shr.template get<int>(), 100);
...
TEST(Shrinkable, with)
{
    Shrinkable<int> shr(100);
    EXPECT_EQ(shr.template get<int>(), 100);
    Shrinkable<int> shr2 = shr.with(Stream<Shrinkable<int>>::one(Shrinkable<int>(200)));
    EXPECT_EQ(shr2.template get<int>(), 100);
    EXPECT_EQ(shr2.getShrinks().isEmpty(), false);
    EXPECT_EQ(shr2.getShrinks().getHeadRef<Shrinkable<int>>().template get<int>(), 200);
    EXPECT_EQ(serializeShrinkable(shr), "{value: 100}");
...
TEST(Shrinkable, concatStatic)
{
    Shrinkable<int> shr(100);
    EXPECT_EQ(shr.template get<int>(), 100);
    Shrinkable<int> shr2 = shr.concatStatic(Stream<Shrinkable<int>>::one(Shrinkable<int>(200)));
    EXPECT_EQ(serializeShrinkable(shr), "{value: 100}");
    EXPECT_EQ(serializeShrinkable(shr2), "{value: 100, shrinks: [{value: 200}]}");

    Shrinkable<int> shr3 = shrinkIntegral<int>(2);
    EXPECT_EQ(serializeShrinkable(shr3), "{value: 2, shrinks: [{value: 0}, {value: 1}]}");
    Shrinkable<int> shr4 = shr3.concatStatic(Stream<Shrinkable<int>>::one(Shrinkable<int>(3)));
...
TEST(Shrinkable, concat)
{
    ...
    Shrinkable<int> shr4 = shr3.concat([](const Shrinkable<int>& shr) {
        return Stream<Shrinkable<int>>::one(Shrinkable<int>(shr.template get<int>() + 5));
    });
...
TEST(Shrinkable, andThenStatic)
{
    Shrinkable<int> shr(100);
    EXPECT_EQ(shr.template get<int>(), 100);
    Shrinkable<int> shr2 = shr.andThenStatic(Stream<Shrinkable<int>>::one(Shrinkable<int>(200)));
    EXPECT_EQ(serializeShrinkable(shr), "{value: 100}");
    EXPECT_EQ(serializeShrinkable(shr2), "{value: 100, shrinks: [{value: 200}]}");

    Shrinkable<int> shr3 = shrinkIntegral<int>(2);
    EXPECT_EQ(serializeShrinkable(shr3), "{value: 2, shrinks: [{value: 0}, {value: 1}]}");
    Shrinkable<int> shr4 = shr3.andThenStatic(Stream<Shrinkable<int>>::one(Shrinkable<int>(3)));
...
TEST(Shrinkable, filter)
{
    ...
    Shrinkable<int> shr2 = shr.filter<int>([](const int& val) { return val == 100; });
    Shrinkable<int> shr3 = shr.filter<int>([](const int& val) { return val <= 200; });
...
TEST(Shrinkable, map)
{
    Shrinkable<int> shr(100);
    EXPECT_EQ(shr.template get<int>(), 100);
    Shrinkable<int> shr2 = shr.map<int,int>([](const int& val) { return val + 1; });
    EXPECT_EQ(101, shr2.template get<int>());
    EXPECT_EQ(serializeShrinkable(shr), "{value: 100}");
...
TEST(Shrinkable, flatMap)
{
    Shrinkable<int> shr(100);
    EXPECT_EQ(shr.template get<int>(), 100);
    Shrinkable<string> shr2 = shr.flatMap<string,int>([](const int& val) { return make_shrinkable<string>(to_string(val + 1)); });
    EXPECT_EQ("101", shr2.template get<string>());
    EXPECT_EQ(serializeShrinkable(shr), "{value: 100}");
...
TEST(Shrinkable, filter_exhaustive)
{
    using Shr = Shrinkable<int64_t>;
    using Str = Stream<Shr>;
    Shr shr = util::binarySearchShrinkable(8).filter<int64_t>([](const int64_t& val) { return val > 4; });
    printExhaustive(shr);
    EXPECT_TRUE(compareShrinkable(shr, Shr(8).with(Str::of<Shr>(
        Shr(6).with(Str::of<Shr>(
            Shr(5)
    ...
    Shrinkable<int64_t> shr2 = util::binarySearchShrinkable(8).filter<int64_t>([](const int64_t& val) { return val > 5; });
...
TEST(Shrinkable, map_exhaustive)
{
    ...
    EXPECT_EQ(serializeShrinkable(shr), "{value: 8, shrinks: [{value: 0}, {value: 4, shrinks: [{value: 2, shrinks: [{value: 1}]}, {value: 3}]}, {value: 6, shrinks: [{value: 5}]}, {value: 7}]}");
    Shrinkable<int> shr2 = shr.map<int,int64_t>([](const int64_t& val) -> int { return static_cast<int>(val + 1); });
    ...
    Shrinkable<string> shr3 = shr.map<string,int64_t>([](const int& val) -> string { return to_string(val); });
...
TEST(Shrinkable, map_exhaustive2)
{
    ...
    Shrinkable<int64_t> shr = util::binarySearchShrinkable(8);
    EXPECT_EQ(serializeShrinkable(shr), "{value: 8, shrinks: [{value: 0}, {value: 4, shrinks: [{value: 2, shrinks: [{value: 1}]}, {value: 3}]}, {value: 6, shrinks: [{value: 5}]}, {value: 7}]}");
    Shrinkable<int> shr2 = shr.map<int,int64_t>(Functor{5});
...
TEST(ShrinkableAny, with)
{
    ...
    EXPECT_EQ(shr.getAny().getRef<int>(), 100);
    ShrinkableAny shr2 = shr.with(Stream<ShrinkableAny>::one(ShrinkableAny(Shrinkable<int>(200))));
...
TEST(ShrinkableAny, map)
{
    ShrinkableAny shr(util::binarySearchShrinkable(8));
    EXPECT_EQ(shr.getAny().getRef<int64_t>(), 8);
    Shrinkable<int> shr2 = shr.map<int,Any>([](const Any& val) -> int { return static_cast<int>(val.getRef<int64_t>() + 1); });
...
TEST(ShrinkableAny, flatMap)
{
    ShrinkableAny shr(util::binarySearchShrinkable(8));
    EXPECT_EQ(shr.getAny().getRef<int64_t>(), 8);
    Shrinkable<string> shr2 = shr.flatMap<string,Any>([](const Any& val) -> Shrinkable<string> { return make_shrinkable<string>(to_string(val.getRef<int64_t>() + 1)); });
...
TEST(ShrinkableAny, andThenStatic)
{
    ShrinkableAny shr(util::binarySearchShrinkable(8));
    EXPECT_EQ(shr.getAny().getRef<int64_t>(), 8);
    ShrinkableAny shr2 = shr.andThenStatic(Stream<ShrinkableAny>::one(ShrinkableAny(Shrinkable<int64_t>(200))));
    EXPECT_EQ(serializeShrinkableAny<int64_t>(shr), "{value: 8, shrinks: [{value: 0}, {value: 4, shrinks: [{value: 2, shrinks: [{value: 1}]}, {value: 3}]}, {value: 6, shrinks: [{value: 5}]}, {value: 7}]}");
...
TEST(ShrinkableAny, andThen)
{
    ShrinkableAny shr(util::binarySearchShrinkable(8));
    EXPECT_EQ(shr.getAny().getRef<int64_t>(), 8);
    ShrinkableAny shr2 = shr.andThen([](const ShrinkableAny& shr) {
...
TEST(ShrinkableAny, concatStatic)
{
    ShrinkableAny shr(util::binarySearchShrinkable(8));
    EXPECT_EQ(shr.getAny().getRef<int64_t>(), 8);
    ShrinkableAny shr2 = shr.concatStatic(Stream<ShrinkableAny>::one(ShrinkableAny(Shrinkable<int64_t>(200))));
    EXPECT_EQ(serializeShrinkableAny<int64_t>(shr), "{value: 8, shrinks: [{value: 0}, {value: 4, shrinks: [{value: 2, shrinks: [{value: 1}]}, {value: 3}]}, {value: 6, shrinks: [{value: 5}]}, {value: 7}]}");
...
TEST(ShrinkableAny, concat)
{
    ShrinkableAny shr(util::binarySearchShrinkable(8));
    EXPECT_EQ(shr.getAny().getRef<int64_t>(), 8);
    ShrinkableAny shr2 = shr.concat([](const ShrinkableAny& shr) {
        return Stream<ShrinkableAny>::one(ShrinkableAny(Shrinkable<int64_t>(100 + shr.getAny().getRef<int64_t>())));
    });
    EXPECT_EQ(serializeShrinkableAny<int64_t>(shr), "{value: 8, shrinks: [{value: 0}, {value: 4, shrinks: [{value: 2, shrinks: [{value: 1}]}, {value: 3}]}, {value: 6, shrinks: [{value: 5}]}, {value: 7}]}");
