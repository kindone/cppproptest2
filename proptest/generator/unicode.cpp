...
Shrinkable<uint32_t> UnicodeGen::operator()(Random& rand)
{
    ...
    auto frontGen = gen::weightedGen<uint32_t>(interval<uint32_t>(0x1, 0xD7FF), static_cast<double>(frontSize) / size);
    auto rearGen = gen::weightedGen<uint32_t>(interval<uint32_t>(0xE000, 0x10FFFF), static_cast<double>(rearSize) / size);
